...
import seedu.addressbook.data.person.Faculty;
...
public class ParserTest {
    ...
    public void parse_addCommandInvalidArgs_errorMessage() {
        final String[] inputs = {
            ...
            // no phone prefix
            String.format("add $s $s e/$s f/$s a/$s", Name.EXAMPLE, Phone.EXAMPLE, Email.EXAMPLE, Faculty.EXAMPLE, Address.EXAMPLE),
            // no email prefix
            String.format("add $s p/$s $s f/$s a/$s", Name.EXAMPLE, Phone.EXAMPLE, Email.EXAMPLE, Faculty.EXAMPLE, Address.EXAMPLE),
            // no faculty prefix
            String.format("add $s p/$s e/$s $s a/$s", Name.EXAMPLE, Phone.EXAMPLE, Email.EXAMPLE, Faculty.EXAMPLE, Address.EXAMPLE),
            // no address prefix
            String.format("add $s p/$s e/$s f/$s $s", Name.EXAMPLE, Phone.EXAMPLE, Email.EXAMPLE, Faculty.EXAMPLE, Address.EXAMPLE)
        };
    ...
    public void parse_addCommandInvalidPersonDataInArgs_errorMessge() {
        ...
        final String invalidFacultyArg = "p/not__alphabets";
        final String validFacultyArg = "p/" + Faculty.EXAMPLE;
        ...
        final String[] inputs = {
                // invalid name
                String.format(addCommandFormatString, invalidName, validPhoneArg, validEmailArg, validFacultyArg),
                // invalid phone
                String.format(addCommandFormatString, validName, invalidPhoneArg, validEmailArg, validFacultyArg),
                // invalid email
                String.format(addCommandFormatString, validName, validPhoneArg, invalidEmailArg, validFacultyArg),
                // invalid faculty
                String.format(addCommandFormatString, validName, validPhoneArg, validEmailArg, invalidFacultyArg),
                // invalid tag
                String.format(addCommandFormatString, validName, validPhoneArg, validEmailArg, validFacultyArg) + " " + invalidTagArg
        };
    ...
    private static Person generateTestPerson() {
        try {
            return new Person(
                ...
                new Faculty(Faculty.EXAMPLE, true),
    ...
    private static String convertPersonToAddCommandString(ReadOnlyPerson person) {
        String addCommand = "add "
                ...
                + (person.getFaculty().isPrivate() ? " pf/" : " f/") + person.getFaculty().value
